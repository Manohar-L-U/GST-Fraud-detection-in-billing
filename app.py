...
def predict():
    ...
    if request.method == "POST":
        try:
            ...
            # Collect user inputs for all model features
            user_input = {}
            ...
            if 1 <= trans_id <= 25001:
                ...
                result = label_map[y_pred]
                probability_non_fraud = round(y_proba, 2)

        except Exception as e:
            flash(f"Error in prediction: {e}", "danger")

    return render_template("predict.html", result=result, probability=probability_non_fraud, columns=train_columns)
